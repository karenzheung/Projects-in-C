...
int bits_io_read_bit (BitsIOFile *bfile) {
  ...
  unsigned char mask = (1<<7);  //mask to get the highest bit
  if(bfile->byte == ALL_BITS_READ){
    int shiftVal = 1;
    int d = fgetc(bfile->fp); 
    unsigned char c = d;
    ...
    if(d  == -1)
      ...
      bfile->count++;
      if((c & mask) ==0){     //0abcdefg
        readByte = (c<<1)|1; // abcdefg1
      }
      else {
        while((c & mask)!=0){     //1110abcd
          c = (c<<1) | shiftVal;  //110abcd1
          shiftVal = 0;           //first time shift 1 then shift in 0 10abcd10
        }                         //0abcd100
        readByte = (c<<1);       //abcd1000
      }
    ...
    unsigned char extractedBit = bfile->byte;  //abcdefg1
    bfile->byte = (extractedBit << 1); //bcdefg10 removed high bit

    return ((extractedBit & mask) >> 7);  // extract 'a'
